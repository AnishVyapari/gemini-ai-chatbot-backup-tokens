...
# ============================================================================
# /GENERATEIMAGE COMMAND - MISTRAL AI AGENT INTEGRATION
# ============================================================================
@bot.tree.command(name="generateimage", description="Generate an image using AI Agent")
@app_commands.describe(prompt="Describe the image you want to generate")
async def slash_generateimage(interaction: discord.Interaction, prompt: str):
    """Generate an image using Mistral AI Agent"""
    try:
        await interaction.response.defer()
        
        # Call Mistral AI Agent API for image generation
        agent_id = "ag_019afdbd54187057916cff91c8b5775a"
        agent_api_url = f"https://api.mistral.ai/agents/v1/agents/{agent_id}/run"
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            response = await client.post(
                ...
                json={
                    "message": f"Generate an image with this prompt: {prompt}"
                }
            )
            response.raise_for_status()
            result = response.json()
        
        # Extract image URL or generated content from agent response
        ...
        if agent_response:
            embed.add_field(name="Response", value=agent_response[:1024], inline=False)
        
        embed.set_footer(text="Generated by Mistral AI Agent")
